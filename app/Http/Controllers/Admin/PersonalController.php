...
class PersonalController extends Controller{
    ...
    public function index(){
        ...
        $datas = User::getAdminLists($map);
        $pages = array(
            'username' => $username
        );
        return view('admin.user.personal_index',compact('datas','pages'));
    }
    ...
    public function add(){
        if(Request::ajax()){
            $view = view('admin.user.personal_add');
            return Response::json(array('html'=>$view->render(),'title'=>'新增用户','status'=>1));
        }else{
    ...
    public function edit($id){
        ...
        return view('admin.user.personal_edit',compact('info'));
    }
    ...
    public function update(PersonalRequest $request){
        ...
        if($res){
            ...
            return Response::json(array('error'=> '用户信息添加失败','status'=>0));
        }
    }
    ...
    public function post(HttpRequest $request){
        ...
        if($resualt){
            return redirect('admin/personal/index')->withSuccess('用户信息修改成功!');
        }else{
            return redirect()->back()->with('error','用户信息修改失败');
        }
    }
